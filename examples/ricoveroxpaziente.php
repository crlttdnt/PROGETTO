...
<header>
    ...
    <div class="container-xl pt-3">
        <nav class="navbar navbar-expand-lg navbar-light justify-content-between">
            ...
            <a class="btn btn-outline-light" href="../php/login.php">
                Login
    ...
    <section class="container-xl mx-auto my-5">
        ...
        <h5 class='  fw-medium '> Visualizza lo storico dei ricoveri di paziente: </h5>
...
function showTable($pazientericoverato)
{
    global $conn;
    ...
    WHERE pazientericoverato = $1
    QRY;

    try {
        $results = pg_query_params($conn, $query, array($pazientericoverato));
        if (!$results) {
    ...
    function showSelect($selected)
    {
        ...
        $query = "SELECT * FROM pazientericoverato";
        ...
        $visualized = array_map(fn ($el) => "Paziente CF: {$el['codicefiscale']}", $results);
        //- Data Ricovero {$el['dataricovero']} , Data Dimissione {$el['datadimissione']}, Reparto {$el['nomereparto']} , Ospedale {$el['codice']} , Numero Stanza {$el['numerostanza']}
